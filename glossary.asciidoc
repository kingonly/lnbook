...
Custody/Custodial::
    Since private keys are what allow bitcoin to be spent, anyone with access to a private key has control over the bitcoins at the associated address. Whenever the rightful owners of bitcoin entrust their private keys (and control over their funds) to another party, that other party is a custodian. This applies to on-chain and off-chain bitcoins.
    The principal reason for custodial services is to spare users the trouble of dealing with keys. The principal criticism of custodial services is that they effectively reproduce the relationships of trust, dependence, and centrality present in the fiat money system.
